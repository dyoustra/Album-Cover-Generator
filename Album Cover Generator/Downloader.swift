...
class Downloader: NSObject, ObservableObject {
    ...
    func waitUntilDone() throws -> URL {
        ...
        default: throw("Should never happen lol" as Error)
        }
    }
...
extension Downloader: URLSessionDelegate, URLSessionDownloadDelegate {
    ...
    func urlSession(_: URLSession, downloadTask _: URLSessionDownloadTask, didFinishDownloadingTo location: URL) {
        guard let path = Path(url: location) else {
            downloadState.value = .failed("Invalid download location received: \(location)" as Error)
            return
        }
        guard let toPath = Path(url: destination) else {
            downloadState.value = .failed("Invalid destination: \(destination)" as Error)
            return
        }
        do {
