...
class GenerationContext: ObservableObject {
    ...
    @Published var seed = Double.random(in: 1.0...1000.0)
    @Published var guidanceScale = 8.0
    ...
    func generate() async throws -> GenerationResult {
        guard let pipeline = pipeline else { throw("No pipeline" as Error) }
    ...
    func cancelGeneration() {
        ...
        self.state = .userCanceled
    }
    
    func startupGeneration() {
        pipeline?.setupCancel()
    }
}
